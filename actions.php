...
function checkavailability($dbconn, $chp_id)
{
    // Query the database to check if the charging point exists and gets policy
    $QueryString = "SELECT count(*)=1 FROM chp_charging_point where chp_id='$chp_id' and chp_top_available=1 ;";

    $result = pg_query($dbconn, $QueryString);

    if (!$result) {
        echo "<br>There is a possible error, unable to know if available or not .\n";
        exit;
    }
